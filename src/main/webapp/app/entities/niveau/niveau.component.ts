...
export class NiveauComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.niveauService
      ...
      .subscribe(
        (res: HttpResponse<INiveau[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInNiveaus(): void {
    this.eventSubscriber = this.eventManager.subscribe('niveauListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: INiveau[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
