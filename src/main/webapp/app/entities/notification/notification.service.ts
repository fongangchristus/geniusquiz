...
export class NotificationService {
  ...
  protected convertDateFromClient(notification: INotification): INotification {
    const copy: INotification = Object.assign({}, notification, {
      dateNotif: notification.dateNotif && notification.dateNotif.isValid() ? notification.dateNotif.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateNotif = res.body.dateNotif ? moment(res.body.dateNotif) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((notification: INotification) => {
        notification.dateNotif = notification.dateNotif ? moment(notification.dateNotif) : undefined;
      });
    }
    return res;
  }
