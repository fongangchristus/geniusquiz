...
export class ChapitreComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.chapitreService
      ...
      .subscribe(
        (res: HttpResponse<IChapitre[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IChapitre): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInChapitres(): void {
    this.eventSubscriber = this.eventManager.subscribe('chapitreListModification', () => this.loadPage());
  }

  delete(chapitre: IChapitre): void {
    const modalRef = this.modalService.open(ChapitreDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IChapitre[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
